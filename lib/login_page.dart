...
void _toggleTorch() async {
    if (Platform.isAndroid) {
      // Попередження для Android
      _showLoginResultDialog(false, message: 'Torch functionality is not supported on Android.');
      return;
    }

    // Перемикаємо стан ліхтарика
    if (_isTorchOn) {
      await FlutterLightPlugin.onLight();  // Виключаємо ліхтарик
    } else {
      await FlutterLightPlugin.onLight();  // Вмикаємо ліхтарик
    }
